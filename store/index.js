...
export const getGenres = createAsyncThunk("netflix/genres", async() => {
    const {
        data: {genres}} =  await axios.get(`${TMDB_BASE_URL}/genre/movie/list?api_key=${API_KEY}`
    );
    return genres;
}); 
...
const getRawData = async(api, genres, paging) => {
    const moviesArray = [];
    for(let i=1; moviesArray.length < 60 && i<10; i++){ // minimum number of movies is 60
...
export const getUserLikedMovies = createAsyncThunk("neftflix/getLiked", async (email) => {
    const { data: {movies}} = await axios.get(`http://localhost:4000/api/user/liked/${email}`);
    return movies;
})

export const removeFromLikedMovies = createAsyncThunk("neftflix/deleteLiked", async ({email, movieId}) => {
    const { data: {movies}} = await axios.put(`http://localhost:4000/api/user/delete`, {
        email, movieId
    });
    return movies;
})
